...
public class Production : MenuManager {
	...
	private void Start () {
		...
		for (var i = 0; i < Screen.height / cellSizeY - 1; i++) { // screen height divided by height of a button gives us how many object that we are gonna need to fill the screen
			for (int j = 0; j < 2; j++){
				createButtons (j);
			}
			SetLastButtons();
	...
	private void createButtons (int i) {
		var clone = Instantiate(firstTwoButton[i], transform);
		...
		clone.name = firstTwoButton[i].name; //when we call it later in building a building we use its name so we give it a proper name here
